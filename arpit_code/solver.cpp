 ...
 */
float trip_distance_travelled(Trip t) {
    float dist = 0;
...
public:
    ...
    HCState(Helicopter h) {
        this->h = h;
    }

    vector<HCState> get_successors() {
        return vector<HCState> (); 
    }
...
void hcrr(SW wc, HCState cstate, HCSpace space, bool red=true) {
    while (wc.check_term()) {
        vector<HCState> successors = cstate.get_successors();
        float obj_fn = std::numeric_limits<float>::min();
        ...
        float next_best_obj_fn = eval_state(bs_state);
        float curr_state_obj_fn = eval_state(cstate);
