...
public:
    ...
    double wcap;
    double weight;
    TripState(const vector<Drop> &drops, double wcap, double weight)
    {
        this->drops = drops;
        this->wcap = wcap;
        this->weight = weight;
...
TripState sampler(vector<Drop> drops, double wcap)
{
    ...
    for (size_t d_idx = 0; d_idx < ts.drops.size(); d_idx++)
    {
        ...
        PackageInfo pack = packs[pack_idx];
...
TripState hcrr_sub(TripState currts, TripState global_state, int rem_restarts, int depth = 0, int max_depth = 1000)
{
    if (depth >= max_depth)
    {
        if (rem_restarts > 0)
        {
            return hcrr_sub(currts, global_state, rem_restarts - 1, 0, max_depth);
        }
        return global_state;
    }
...
void assignPackages2(Trip &t, double wcap)
{
    ...
    vector<Drop> d = t.drops;
    TripState ts = sampler(d, wcap);
    TripState best = hcrr_sub(ts, ts, 10, 0, 900);
    t.drops = best.drops;
...
//                 if (!global_fits)
//                     break; // No need to check local if global fails

//                 // ***MODIFIED: Check the new, specific local constraints***
//                 // cout << "wcap" << w << endl;
//                 bool local_fits = false;
//                 if (pack.id == 0 || pack.id == 1)
//                 { // Component is a1 or a2
...
//                     if (pack.id == 0)
//                         drops[d_idx].dry_food += 1;
//                     else if (pack.id == 1)
//                         drops[d_idx].perishable_food += 1;
//                     else
//                         drops[d_idx].other_supplies += 1;
...
public:
    ...
    {
        HCState state;
        for (auto &heli : data.helicopters)
        {
